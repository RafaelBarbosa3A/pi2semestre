...
public class TelaEditarCliente extends javax.swing.JFrame {
    ...
    public void setCliente(Cliente cli) {
        ...
        if (cli.getRG() != 0) {
            campoRG.setText(Integer.toString(cliente.getRG()));
        }
        campoCPF.setText(Long.toString(cliente.getCPF()));
        for (int i = 0; i < caixaSexo.getItemCount(); i++) {
            if (caixaSexo.getItemAt(i).equals(cliente.getSexo())) {
                caixaSexo.setSelectedIndex(i);
                break;
            }
        }
        if (cli.getTelefone() != 0) {
            campoTelefone.setText(Integer.toString(cliente.getTelefone()));
        }
        campoEmail.setText(cliente.getEmail());
        campoEndereco.setText(cliente.getEndereço());
        campoNumero.setText(Integer.toString(cliente.getNumero()));
        campoComp.setText(cliente.getComplemento());
        if (cli.getCEP() != 0) {
            campoCEP.setText(Integer.toString(cliente.getCEP()));
        }
